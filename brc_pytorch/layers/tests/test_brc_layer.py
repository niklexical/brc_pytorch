...
def test_grad_flow(generate_dataset, batch_first, bidirectional, num_layers):
    ...
    recurrent_layers = [BistableRecurrentCell(input_size, hidden_size)]
    inner_input_dimensions = num_directions * hidden_size

    for _ in range(num_layers - 1):
        recurrent_layers.append(
            BistableRecurrentCell(inner_input_dimensions, hidden_size)
        )

    # return_seq = False, since we only want the output from the last time step.
    rnn = MultiLayerBase(
        "BRC",
        recurrent_layers,
        hidden_size,
    ...
    # model = nn.Sequential(rnn, nn.Linear(num_directions * hidden_size, 1))
    ...
    assert len(
        param_groups_model
    ) == num_layers * 7 * num_directions + 2, f'{param_groups_model}'

    assert sum(numweights_model) == (
        (3 * input_size * hidden_size + 4 * hidden_size) * num_directions +
        (3 * hidden_size * num_directions * hidden_size + 4 * hidden_size) *
        (num_layers - 1) * num_directions + num_directions * hidden_size + 1
    ), f'{numweights_model}'
