...
from examples.brc_benchmark1 import SelectItem
...
def test_multilayer_rnn(generate_dataset):
    ...
    model_torch = nn.Sequential(
        nn.GRU(input_size, 100, 2), SelectItem(0),
    ...
    for e in range(epochs):
        ...
        for idx, (x_batch, y_batch) in enumerate(training_loader):
            ...
            train_loss_torch = loss_fn(pred_train_torch, y_batch)
    ...
    my_test_loss = loss_fn(my_test_pred, test_y)

    model_torch.eval()
    torch_test_pred = model_torch(test_x)
    torch_test_loss = loss_fn(torch_test_pred, test_y)

    assert my_test_loss <= torch_test_loss
