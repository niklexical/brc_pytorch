...
class NeuromodulatedBistableRecurrentCellLayer(nn.Module):
    """Neuromodulated Bistable Recurrent Cell Layer (Vecoven et al. 2020)"""
    ...
    def __init__(self, input_size, output_dim, *args, **kwargs):
        ...
        """
        super(NeuromodulatedBistableRecurrentCellLayer,
        ...
        self.memoryz = nn.Parameter(
            torch.FloatTensor(self.output_dim, self.output_dim)
        )
        self.memoryr = nn.Parameter(
            torch.FloatTensor(self.output_dim, self.output_dim)
        )
        ...
        nn.init.orthogonal_(self.memoryz)
        nn.init.orthogonal_(self.memoryr)
    ...
    def forward(
        ...
        z = torch.sigmoid(
            torch.matmul(input_t, self.kernelz) +
            torch.matmul(hidden_state, self.memoryz) + self.bz
        )
        r = torch.tanh(
            torch.matmul(input_t, self.kernelr) +
            torch.matmul(hidden_state, self.memoryr) + self.br
        ) + 1
        new_hidden_state = torch.tanh(
            torch.matmul(input_t, self.kernelh) + r * hidden_state
        )
        output_hidden_state = z * hidden_state + (1.0 - z) * new_hidden_state
